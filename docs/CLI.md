...
go run ./apps/cli/cmd/mcphubctl --api-url http://localhost:4000 client config --client opencode --server 00000000-0000-4000-8000-000000000102 --output json
...
Commands include login/logout/whoami, server, tool, grant, policy, audit search/export, client config/test, health, doctor, version, and shell completion. `audit export` requires `--from` and `--to`; use `--signed` to request an API-signed compliance envelope. For local development, `client config` defaults to the seeded `k8s-readonly` server ID when `--server` is omitted.
