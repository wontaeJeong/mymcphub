...
export default async function UserServerDetailPage({ params }: UserServerDetailPageProps) {
  ...
  const grantStatusByToolKey = buildUserToolAccessStatus(server.data, toolItems, grants.ok ? visibleGrants : undefined);
  ...
  const hasAccess = hasActiveServerAccess(visibleGrants);
    ...
    <div className="page-stack">
      ...
      <div className="detail-grid">
        ...
        <Surface className="panel--accent">
          <SectionHeader title="접근 상태" description="현재 세션의 grant와 서버 운영 상태를 함께 해석합니다." />
          <div className="grid">
            <p><strong>내 접근:</strong> {hasAccess ? "활성 권한 있음" : "접근 요청 필요"}</p>
            ...
            {!hasAccess ? <Link className="button" href={accessRequestHref}>접근 요청</Link> : null}
          </div>
        </Surface>
      </div>
      <section>
