...
export async function CatalogPageContent({ searchParams, mode }: CatalogPageProps & Readonly<{ mode: "user" | "admin" }>) {
  ...
  return (
    <div className="page-stack">
      <PageHero eyebrow="MCP 서버 카탈로그" title="신뢰할 서버를 빠르게 찾으세요." description={mode === "admin" ? "실시간 제어 플레인 카탈로그와 상태 엔드포인트에서 MCP 서버를 탐색, 필터링, 등록합니다." : "관리자 전용 제어 없이 실시간 제어 플레인 카탈로그에서 MCP 서버를 탐색하고 필터링합니다."} />
      ...
      <section>
        <SectionHeader title="카탈로그" description="서버 목록에는 슬러그, 표시 이름, 소유 팀, 환경, 전송 방식, 위험도, 상태, 활성 여부가 포함됩니다." />
