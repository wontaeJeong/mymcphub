...
export async function CatalogPageContent({ searchParams, mode }: CatalogPageProps & Readonly<{ mode: "user" | "admin" }>) {
  ...
  return (
    <div className="page-stack">
      <PageHero eyebrow={mode === "user" ? "MCP Market" : "MCP 서버 카탈로그"} title={mode === "user" ? "내부 MCP 서버를 발견하고 연결하세요." : "신뢰할 서버를 빠르게 찾으세요."} description={mode === "admin" ? "실시간 제어 플레인 카탈로그와 상태 엔드포인트에서 MCP 서버를 탐색, 필터링, 등록합니다." : "검증 상태, 접근 가능 여부, 운영 상태를 함께 보며 필요한 MCP 서버를 찾습니다. 광고, 랭킹, skills 없이 내부 운영 목적의 탐색만 제공합니다."} />
      ...
      <section>
        <SectionHeader title={mode === "user" ? "MCP Market 결과" : "카탈로그"} description={mode === "user" ? "카드는 요약, 카테고리, 태그, 신뢰 수준, 접근 상태, 운영 상태와 다음 행동을 함께 보여줍니다." : "서버 목록에는 슬러그, 표시 이름, 소유 팀, 환경, 전송 방식, 위험도, 상태, 활성 여부가 포함됩니다."} />
