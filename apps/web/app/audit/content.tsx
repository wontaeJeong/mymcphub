...
import { ExternalLinkIcon, FilterIcon, KeyIcon, SearchIcon } from "../../components/icons";
...
export async function AuditPageContent({ searchParams }: AuditPageProps) {
  ...
  return (
    <div className="page-stack">
      ...
      <form className="form-card filter-card" action="/admin/audit">
        ...
        <details className="schema-viewer">
          <summary><FilterIcon />고급 진단 필터</summary>
          <div className="filter-grid">
        ...
        <div className="form-actions">
          ...
          <a className="button button--subtle" href="/admin/audit">초기화</a>
        </div>
      </form>
      <section>
        <SectionHeader title="정책 및 관리자 이벤트" description="정책 결정과 관리자 행위를 요약하고 추적·페이로드 진단은 행 안에서 펼칩니다." action={<div className="actions"><a className="button button--ghost button--compact" href={exportHref}><ExternalLinkIcon />필터 결과 JSON 내보내기</a>{nextAuditHref ? <Link className="button button--compact" href={nextAuditHref}><KeyIcon />다음 페이지</Link> : null}</div>} />
