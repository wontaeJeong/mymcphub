...
export async function AuditPageContent({ searchParams }: AuditPageProps) {
  ...
  return (
    <div className="page-stack">
      ...
      <section>
        <SectionHeader title="정책 및 관리자 이벤트" description="서버 측 필터와 마스킹된 인자 세부 정보가 포함된 /api/audit-events 페이지 결과입니다." action={<div className="actions"><a className="button button--ghost" href={exportHref}>필터 결과 JSON 내보내기</a>{nextAuditHref ? <Link className="button" href={nextAuditHref}>다음 페이지</Link> : null}</div>} />
