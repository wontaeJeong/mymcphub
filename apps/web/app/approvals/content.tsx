...
export async function ApprovalsPageContent() {
  ...
  return (
    <div className="page-stack">
      <PageHero eyebrow="승인 요청" title="승인 요청 검토" description="요청 사유, 도구 위험도, 기존 권한을 확인한 뒤 승인 여부를 결정합니다." />
      {!servers.ok ? <ErrorState title="서버 정보를 확인할 수 없습니다" message={servers.error} /> : null}
      {!grants.ok ? <ErrorState title="기존 권한을 확인할 수 없습니다" message={grants.error} /> : null}
      <section>
        <SectionHeader title="대기 중인 요청" description="요청 맥락을 확인한 뒤 승인 또는 거절합니다." />
      ...
      <section>
        <SectionHeader title="결정 기록" description="최근 승인 및 거절 내역입니다." />
          ...
          <Surface key={approval.id}>
            <div className="section-header">
              <div>
                ...
                <h2>{context?.serverDisplayName ?? serverNameById.get(approval.serverId) ?? "서버 정보 없음"}</h2>
                <p>{approval.reason}</p>
              </div>
            ...
            <div className="detail-grid">
              ...
              <div className="grid">
                ...
                <p><strong>기존 권한:</strong> {context ? `${context.grantOverlaps.length}개` : "정보 없음"}</p>
                ...
                ) : <p className="muted">도구 위험도 정보가 없습니다.</p>}
              </div>
            </div>
