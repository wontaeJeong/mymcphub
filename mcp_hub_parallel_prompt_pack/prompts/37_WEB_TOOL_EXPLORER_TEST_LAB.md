...
MCP tool schema를 보고 제한된 test call을 실행할 수 있는 UI를 구현한다.
...
1. input schema form를 구현하거나 보강한다.
2. dry-run mode를 구현하거나 보강한다.
3. admin-only execute를 구현하거나 보강한다.
4. redacted result를 구현하거나 보강한다.
5. copy JSON를 구현하거나 보강한다.
...
- 관리자는 tool schema와 호출 결과를 UI에서 검증할 수 있다.
...
`docs/handoffs/37_WEB_TOOL_EXPLORER_TEST_LAB.md` 파일을 만들고 다음을 기록한다.

```md
# 37. Tool Explorer and Test Lab Handoff
