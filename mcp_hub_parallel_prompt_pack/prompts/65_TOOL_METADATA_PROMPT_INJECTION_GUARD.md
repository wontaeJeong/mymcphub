# 65. Tool Metadata Prompt-injection Guard
...
tool description/schema에 포함된 prompt-injection 위험 문구를 탐지하는 scanner를 구현한다.

## 주요 소유 경로

- `packages/security/src/tool-metadata-scan/**`
- `apps/worker/src/tool-scan/**`
...
1. rule set를 구현하거나 보강한다.
2. severity를 구현하거나 보강한다.
3. quarantine suggestion를 구현하거나 보강한다.
4. false positive override를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- 위험한 tool metadata가 scan 결과에 표시되고 자동 publish를 막을 수 있다.
...
`docs/handoffs/65_TOOL_METADATA_PROMPT_INJECTION_GUARD.md` 파일을 만들고 다음을 기록한다.

```md
# 65. Tool Metadata Prompt-injection Guard Handoff
