# 50. Secret Broker Runtime Injection
...
MCP server runtime에 short-lived credential 또는 secret reference를 주입하는 구조를 구현한다.

## 주요 소유 경로

- `packages/runtime/src/secrets/**`
- `apps/worker/src/secrets/**`
...
1. provider interface를 구현하거나 보강한다.
2. no raw secret logs를 구현하거나 보강한다.
3. env/file injection를 구현하거나 보강한다.
4. rotation hook를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- runtime manifest는 raw secret 대신 secret reference를 받아 안전하게 주입한다.
...
`docs/handoffs/50_SECRET_BROKER_RUNTIME_INJECTION.md` 파일을 만들고 다음을 기록한다.

```md
# 50. Secret Broker Runtime Injection Handoff
