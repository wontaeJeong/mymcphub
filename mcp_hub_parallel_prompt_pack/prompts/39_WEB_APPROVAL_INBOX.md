...
승인자가 pending grant 요청을 검토/승인/거절하는 UI를 구현한다.
...
1. pending list를 구현하거나 보강한다.
2. diff/risk를 구현하거나 보강한다.
3. approve/deny를 구현하거나 보강한다.
4. comment를 구현하거나 보강한다.
5. bulk-safe design를 구현하거나 보강한다.
...
- 승인자는 요청 사유와 위험도를 보고 접근 권한을 처리할 수 있다.
...
`docs/handoffs/39_WEB_APPROVAL_INBOX.md` 파일을 만들고 다음을 기록한다.

```md
# 39. Approval Inbox Handoff
