...
argument/result/log/audit에서 민감정보를 마스킹하는 redaction library를 구현한다.
...
1. token patterns를 구현하거나 보강한다.
2. kubeconfig를 구현하거나 보강한다.
3. email/url를 구현하거나 보강한다.
4. custom regex를 구현하거나 보강한다.
5. unit tests를 구현하거나 보강한다.
...
- 감사로그와 애플리케이션 로그에 secret 원문이 남지 않는다.
...
`docs/handoffs/66_SENSITIVE_DATA_REDACTION.md` 파일을 만들고 다음을 기록한다.

```md
# 66. Sensitive Data Redaction Handoff
