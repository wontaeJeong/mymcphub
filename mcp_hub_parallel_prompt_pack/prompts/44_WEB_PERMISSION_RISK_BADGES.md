# 44. Permission Visibility and Risk Badges
...
UI 전반에 권한 상태와 risk level을 일관되게 표시하는 컴포넌트를 구현한다.

## 주요 소유 경로

- `packages/ui/src/permissions/**`
- `apps/web/**`
...
1. risk badge를 구현하거나 보강한다.
2. grant status를 구현하거나 보강한다.
3. tool hidden reason를 구현하거나 보강한다.
4. tooltip를 구현하거나 보강한다.
5. accessibility를 구현하거나 보강한다.
...
- 사용자는 어떤 tool이 왜 보이거나 숨겨졌는지 이해할 수 있다.
...
`docs/handoffs/44_WEB_PERMISSION_RISK_BADGES.md` 파일을 만들고 다음을 기록한다.

```md
# 44. Permission Visibility and Risk Badges Handoff
