...
Gateway/API/Worker 간 공유 contract test를 추가한다.
...
1. OpenAPI validation를 구현하거나 보강한다.
2. tool schema fixtures를 구현하거나 보강한다.
3. policy decision fixtures를 구현하거나 보강한다.
4. CI command를 구현하거나 보강한다.
...
- 공유 schema 변경이 contract test로 검증된다.
...
`docs/handoffs/71_CONTRACT_TESTS.md` 파일을 만들고 다음을 기록한다.

```md
# 71. Contract Tests Handoff
