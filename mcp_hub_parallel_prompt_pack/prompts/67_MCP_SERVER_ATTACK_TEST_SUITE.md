# 67. MCP Server Attack Test Suite
...
MCP server용 SSRF/path traversal/command injection 회귀 테스트 harness를 만든다.

## 주요 소유 경로

- `tests/security/**`
- `packages/security/src/harness/**`
...
1. malicious args를 구현하거나 보강한다.
2. expected deny를 구현하거나 보강한다.
3. fixture server를 구현하거나 보강한다.
4. CI target를 구현하거나 보강한다.
5. report를 구현하거나 보강한다.
...
- 신규 MCP server가 기본 공격 시나리오를 통과하는지 검증할 수 있다.
...
`docs/handoffs/67_MCP_SERVER_ATTACK_TEST_SUITE.md` 파일을 만들고 다음을 기록한다.

```md
# 67. MCP Server Attack Test Suite Handoff
