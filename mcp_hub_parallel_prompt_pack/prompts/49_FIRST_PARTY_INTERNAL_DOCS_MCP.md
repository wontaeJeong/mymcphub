# 49. First-party Internal Docs MCP Server
...
내부 문서 검색용 MCP server scaffold를 구현한다.
...
1. search tool를 구현하거나 보강한다.
2. fetch document tool를 구현하거나 보강한다.
3. ACL hook를 구현하거나 보강한다.
4. mock index를 구현하거나 보강한다.
5. redaction를 구현하거나 보강한다.
...
- 문서 검색/fetch tool이 ACL hook을 거쳐 결과를 반환한다.
...
`docs/handoffs/49_FIRST_PARTY_INTERNAL_DOCS_MCP.md` 파일을 만들고 다음을 기록한다.

```md
# 49. First-party Internal Docs MCP Server Handoff
