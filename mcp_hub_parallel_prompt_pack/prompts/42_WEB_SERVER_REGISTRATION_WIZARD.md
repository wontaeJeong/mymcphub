...
관리자가 MCP server를 등록하는 wizard UI를 구현한다.

## 주요 소유 경로

- `apps/web/app/admin/servers/new/**`
...
1. transport 선택를 구현하거나 보강한다.
2. upstream URL를 구현하거나 보강한다.
3. owner를 구현하거나 보강한다.
4. secret requirements를 구현하거나 보강한다.
5. initial scan를 구현하거나 보강한다.
6. publish review를 구현하거나 보강한다.
...
- 관리자는 단계형 wizard로 신규 MCP server를 안전하게 등록할 수 있다.
...
`docs/handoffs/42_WEB_SERVER_REGISTRATION_WIZARD.md` 파일을 만들고 다음을 기록한다.

```md
# 42. Server Registration Wizard Handoff
