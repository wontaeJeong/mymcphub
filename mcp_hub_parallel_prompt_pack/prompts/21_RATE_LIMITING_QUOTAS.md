...
user/team/project/server/tool 단위 rate limit과 quota를 구현한다.

## 주요 소유 경로

- `apps/gateway/src/limits/**`
- `packages/policy/src/limits.ts`
...
1. token bucket를 구현하거나 보강한다.
2. Redis adapter를 구현하거나 보강한다.
3. 429 응답를 구현하거나 보강한다.
4. per-risk limit를 구현하거나 보강한다.
5. metrics hook를 구현하거나 보강한다.
...
- 과도한 MCP 호출은 upstream 도달 전에 제한되고, 제한 이벤트가 metrics/audit로 남는다.
...
`docs/handoffs/21_RATE_LIMITING_QUOTAS.md` 파일을 만들고 다음을 기록한다.

```md
# 21. Rate Limiting and Quotas Handoff
