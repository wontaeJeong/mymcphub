# 48. First-party Git Provider MCP Server
...
GitHub/GitLab 계열 read/write 분리 가능한 MCP server scaffold를 구현한다.

## 주요 소유 경로

- `servers/git-provider/**`
...
2. read-only tools를 구현하거나 보강한다.
3. write tools behind flag를 구현하거나 보강한다.
4. mock provider를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- Git provider MCP server가 provider abstraction과 mock 기반 테스트를 가진다.
...
`docs/handoffs/48_FIRST_PARTY_GIT_PROVIDER_MCP.md` 파일을 만들고 다음을 기록한다.

```md
# 48. First-party Git Provider MCP Server Handoff
