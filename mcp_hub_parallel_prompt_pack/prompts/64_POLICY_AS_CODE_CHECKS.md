...
OPA/Gatekeeper 또는 Conftest 기반 manifest policy check를 추가한다.
...
1. non-root를 구현하거나 보강한다.
2. read-only rootfs를 구현하거나 보강한다.
3. no wildcard egress를 구현하거나 보강한다.
...
- 배포 manifest가 기본 보안 정책을 통과해야 merge된다.
...
`docs/handoffs/64_POLICY_AS_CODE_CHECKS.md` 파일을 만들고 다음을 기록한다.

```md
# 64. Policy-as-code Checks Handoff
