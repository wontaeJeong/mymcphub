# 22. Upstream Circuit Breaker, Timeouts, Retries
...
불안정한 MCP upstream으로 인한 client 지연을 줄이기 위해 timeout/circuit breaker를 구현한다.

## 주요 소유 경로

- `apps/gateway/src/upstream/**`
...
1. connect timeout를 구현하거나 보강한다.
2. request timeout를 구현하거나 보강한다.
3. half-open를 구현하거나 보강한다.
4. exponential backoff를 구현하거나 보강한다.
5. per-server status를 구현하거나 보강한다.
...
- 죽은 upstream에는 일정 기간 요청을 보내지 않고 즉시 명확한 에러를 반환한다.
...
`docs/handoffs/22_UPSTREAM_CIRCUIT_BREAKER_TIMEOUTS.md` 파일을 만들고 다음을 기록한다.

```md
# 22. Upstream Circuit Breaker, Timeouts, Retries Handoff
