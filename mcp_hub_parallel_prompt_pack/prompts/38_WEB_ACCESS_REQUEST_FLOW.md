...
사용자가 MCP server/tool 접근 권한을 요청하는 UI를 구현한다.

## 주요 소유 경로

- `apps/web/app/access-requests/**`
...
1. server 선택를 구현하거나 보강한다.
2. tool scope 선택를 구현하거나 보강한다.
3. project/reason/expires_at를 구현하거나 보강한다.
4. risk warning를 구현하거나 보강한다.
5. submit state를 구현하거나 보강한다.
...
- 사용자는 필요한 MCP 접근 권한을 self-service로 요청할 수 있다.
...
`docs/handoffs/38_WEB_ACCESS_REQUEST_FLOW.md` 파일을 만들고 다음을 기록한다.

```md
# 38. Access Request Flow Handoff
