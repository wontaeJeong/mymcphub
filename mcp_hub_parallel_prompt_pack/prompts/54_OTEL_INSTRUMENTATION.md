# 54. OpenTelemetry Instrumentation
...
Gateway/API/Worker에 공통 OTel tracing을 적용한다.
...
1. trace provider를 구현하거나 보강한다.
2. span naming를 구현하거나 보강한다.
3. trace id propagation를 구현하거나 보강한다.
4. error attributes를 구현하거나 보강한다.
5. sample config를 구현하거나 보강한다.
...
- MCP request 하나가 API/Gateway/upstream span으로 연결되어 추적된다.
...
`docs/handoffs/54_OTEL_INSTRUMENTATION.md` 파일을 만들고 다음을 기록한다.

```md
# 54. OpenTelemetry Instrumentation Handoff
