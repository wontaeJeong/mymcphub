...
MCP server가 필요로 하는 secret type과 runtime binding을 관리하는 API를 구현한다.
...
1. secret_ref를 구현하거나 보강한다.
2. provider type를 구현하거나 보강한다.
3. scope를 구현하거나 보강한다.
4. rotation metadata를 구현하거나 보강한다.
5. no raw secret storage를 구현하거나 보강한다.
6. validation를 구현하거나 보강한다.
...
- API는 raw secret을 반환하지 않고 secret reference/binding metadata만 제공한다.
...
`docs/handoffs/30_SECRET_BINDING_API.md` 파일을 만들고 다음을 기록한다.

```md
# 30. Secret Binding API Handoff
