# 17. Gateway Session Manager and Sticky Routing
...
MCP session을 추적하고 동일 session의 요청을 동일 upstream/runtime으로 라우팅한다.
...
1. session id 생성/검증를 구현하거나 보강한다.
2. TTL를 구현하거나 보강한다.
3. Redis/in-memory adapter를 구현하거나 보강한다.
4. reconnect 처리를 구현하거나 보강한다.
5. session cleanup를 구현하거나 보강한다.
...
- 동일 MCP session id가 같은 upstream instance로 라우팅되고 만료된 session은 재사용되지 않는다.
...
`docs/handoffs/17_GATEWAY_SESSION_MANAGER_STICKY_ROUTING.md` 파일을 만들고 다음을 기록한다.

```md
# 17. Gateway Session Manager and Sticky Routing Handoff
