...
stdio MCP server를 remote endpoint처럼 노출하는 adapter pod scaffold를 구현한다.

## 주요 소유 경로

- `packages/runtime/src/stdio-adapter/**`
- `servers/stdio-adapter-example/**`
...
1. subprocess lifecycle를 구현하거나 보강한다.
2. stdout/stderr separation를 구현하거나 보강한다.
3. timeout를 구현하거나 보강한다.
4. health를 구현하거나 보강한다.
5. example config를 구현하거나 보강한다.
...
- 기존 stdio MCP server를 adapter로 감싸 integration test에서 호출할 수 있다.
...
`docs/handoffs/46_STDIO_ADAPTER_POD_SCAFFOLD.md` 파일을 만들고 다음을 기록한다.

```md
# 46. Stdio Adapter Pod Scaffold Handoff
