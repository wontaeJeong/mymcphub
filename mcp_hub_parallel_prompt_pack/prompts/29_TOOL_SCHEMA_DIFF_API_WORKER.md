# 29. Tool Schema Snapshot and Diff API
...
MCP `tools/list` snapshot 저장과 schema diff 조회 API를 구현한다.
...
1. snapshot table를 구현하거나 보강한다.
2. added/removed/changed를 구현하거나 보강한다.
3. risk recalculation hook를 구현하거나 보강한다.
4. diff API를 구현하거나 보강한다.
5. worker job stub를 구현하거나 보강한다.
...
- 서버 버전별 tool schema 변화가 저장되고 UI/API에서 diff를 볼 수 있다.
...
`docs/handoffs/29_TOOL_SCHEMA_DIFF_API_WORKER.md` 파일을 만들고 다음을 기록한다.

```md
# 29. Tool Schema Snapshot and Diff API Handoff
