...
MCP session/tool call/policy decision 감사 이벤트를 검색하는 API를 구현한다.
...
1. time range를 구현하거나 보강한다.
2. user/team/project를 구현하거나 보강한다.
3. server/tool를 구현하거나 보강한다.
4. decision를 구현하거나 보강한다.
5. pagination를 구현하거나 보강한다.
6. redaction를 구현하거나 보강한다.
...
- 관리자는 대량 audit event를 필터링/페이지네이션으로 조회할 수 있다.
...
`docs/handoffs/33_AUDIT_QUERY_API.md` 파일을 만들고 다음을 기록한다.

```md
# 33. Audit Query API Handoff
