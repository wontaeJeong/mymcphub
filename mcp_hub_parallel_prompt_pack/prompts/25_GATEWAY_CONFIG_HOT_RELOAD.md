...
server registry, policy, limit config 변경을 Gateway 재시작 없이 반영하는 hot reload 구조를 구현한다.
...
1. file/db provider interface를 구현하거나 보강한다.
2. watch/reload를 구현하거나 보강한다.
3. versioned config를 구현하거나 보강한다.
4. invalid config rollback를 구현하거나 보강한다.
5. health endpoint 반영를 구현하거나 보강한다.
...
- config 변경 시 새 요청부터 반영되고 invalid config는 기존 설정을 유지한다.
...
`docs/handoffs/25_GATEWAY_CONFIG_HOT_RELOAD.md` 파일을 만들고 다음을 기록한다.

```md
# 25. Gateway Config Hot Reload Handoff
