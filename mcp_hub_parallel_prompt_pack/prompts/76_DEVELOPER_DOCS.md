...
개발자가 Hub를 빌드/실행/테스트/디버깅하는 문서를 보강한다.
...
1. quickstart를 구현하거나 보강한다.
2. repo layout를 구현하거나 보강한다.
3. env vars를 구현하거나 보강한다.
4. troubleshooting를 구현하거나 보강한다.
5. common commands를 구현하거나 보강한다.
...
- 새 개발자가 로컬에서 전체 스택을 실행하고 테스트할 수 있다.
...
`docs/handoffs/76_DEVELOPER_DOCS.md` 파일을 만들고 다음을 기록한다.

```md
# 76. Developer Documentation Handoff
