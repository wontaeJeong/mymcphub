...
Kata/gVisor/seccomp/non-root/read-only filesystem 등 sandbox profile을 정의한다.
...
1. profile schema를 구현하거나 보강한다.
2. runtimeClassName를 구현하거나 보강한다.
3. securityContext를 구현하거나 보강한다.
4. resource limits를 구현하거나 보강한다.
5. examples를 구현하거나 보강한다.
...
- 위험도별 runtime isolation profile을 workload manifest에 적용할 수 있다.
...
`docs/handoffs/52_RUNTIME_SANDBOX_PROFILES.md` 파일을 만들고 다음을 기록한다.

```md
# 52. Runtime Sandbox Profiles Handoff
