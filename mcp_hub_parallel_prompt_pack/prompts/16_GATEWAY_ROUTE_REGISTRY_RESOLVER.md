# 16. Gateway Route Registry Resolver
...
`/mcp/{serverSlug}` 요청을 등록된 MCP server/upstream으로 해석하는 route resolver를 구현한다.
...
1. server slug 정규화를 구현하거나 보강한다.
2. enabled/disabled 상태 반영를 구현하거나 보강한다.
3. 환경별 upstream 선택를 구현하거나 보강한다.
4. 존재하지 않는 server에 대한 표준 에러를 구현하거나 보강한다.
5. unit test를 구현하거나 보강한다.
...
- 등록된 server slug에 대해 upstream target과 transport metadata를 반환하고, disabled server는 연결 전에 차단된다.
...
`docs/handoffs/16_GATEWAY_ROUTE_REGISTRY_RESOLVER.md` 파일을 만들고 다음을 기록한다.

```md
# 16. Gateway Route Registry Resolver Handoff
