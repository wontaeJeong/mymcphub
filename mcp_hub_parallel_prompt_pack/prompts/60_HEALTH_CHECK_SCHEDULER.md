...
등록된 MCP server의 health와 tool scan을 주기적으로 실행하는 scheduler를 구현한다.
...
1. interval config를 구현하거나 보강한다.
2. concurrency를 구현하거나 보강한다.
3. timeout를 구현하거나 보강한다.
4. last_success를 구현하거나 보강한다.
...
- worker가 MCP server 상태를 주기적으로 갱신하고 실패를 기록한다.
...
`docs/handoffs/60_HEALTH_CHECK_SCHEDULER.md` 파일을 만들고 다음을 기록한다.
