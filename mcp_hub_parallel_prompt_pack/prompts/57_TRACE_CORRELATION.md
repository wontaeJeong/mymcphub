...
audit event, log, metrics, trace를 `trace_id/session_id/tool_call_id`로 상호 연결한다.
...
1. correlation ids를 구현하거나 보강한다.
2. log context를 구현하거나 보강한다.
3. audit fields를 구현하거나 보강한다.
4. trace links를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- 운영자는 audit event에서 관련 trace/log로 이동할 수 있는 id를 얻는다.
...
`docs/handoffs/57_TRACE_CORRELATION.md` 파일을 만들고 다음을 기록한다.
