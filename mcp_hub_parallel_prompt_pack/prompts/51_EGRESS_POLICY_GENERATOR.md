...
MCP server manifest의 allowed egress를 Kubernetes NetworkPolicy로 변환하는 generator를 구현한다.

## 주요 소유 경로

- `packages/runtime/src/network-policy/**`
...
1. allowlist를 구현하거나 보강한다.
2. deny default를 구현하거나 보강한다.
3. dns exceptions를 구현하거나 보강한다.
4. namespace selector를 구현하거나 보강한다.
5. manifest tests를 구현하거나 보강한다.
...
- server별 egress allowlist에서 NetworkPolicy YAML을 생성할 수 있다.
...
`docs/handoffs/51_EGRESS_POLICY_GENERATOR.md` 파일을 만들고 다음을 기록한다.

```md
# 51. Egress Policy Generator Handoff
