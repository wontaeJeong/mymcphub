# 80. Worktree Parallel Merge Guide
...
여러 shell/worktree에서 lane별 작업 후 merge하는 절차를 문서화한다.

## 주요 소유 경로

- `docs/dev/parallel-worktree-merge.md`
- `scripts/dev/**`
...
1. worktree commands를 구현하거나 보강한다.
2. branch naming를 구현하거나 보강한다.
3. merge order를 구현하거나 보강한다.
4. conflict policy를 구현하거나 보강한다.
5. handoff template를 구현하거나 보강한다.
...
- 여러 에이전트가 병렬 작업한 결과를 안정적으로 통합할 수 있다.
...
`docs/handoffs/80_WORKTREE_PARALLEL_MERGE_GUIDE.md` 파일을 만들고 다음을 기록한다.

```md
# 80. Worktree Parallel Merge Guide Handoff
