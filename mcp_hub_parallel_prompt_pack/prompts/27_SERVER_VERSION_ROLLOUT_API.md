# 27. Server Version and Rollout API
...
MCP server version, image digest, config version, rollout 상태를 관리하는 API를 구현한다.
...
1. version entity를 구현하거나 보강한다.
2. active/canary/deprecated를 구현하거나 보강한다.
3. rollback target를 구현하거나 보강한다.
4. image digest를 구현하거나 보강한다.
5. migration를 구현하거나 보강한다.
...
- 서버별 active version과 rollback 후보를 API로 조회/변경할 수 있다.
...
`docs/handoffs/27_SERVER_VERSION_ROLLOUT_API.md` 파일을 만들고 다음을 기록한다.

```md
# 27. Server Version and Rollout API Handoff
