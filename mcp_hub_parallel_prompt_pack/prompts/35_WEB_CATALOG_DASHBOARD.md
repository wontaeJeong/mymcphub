...
MCP server catalog dashboard를 구현한다.
...
1. server list를 구현하거나 보강한다.
2. filter/search를 구현하거나 보강한다.
3. status badge를 구현하거나 보강한다.
4. risk badge를 구현하거나 보강한다.
5. owner/team를 구현하거나 보강한다.
6. empty/error state를 구현하거나 보강한다.
...
- 사용자는 등록된 MCP server 목록과 상태/위험도를 한 화면에서 확인할 수 있다.
...
`docs/handoffs/35_WEB_CATALOG_DASHBOARD.md` 파일을 만들고 다음을 기록한다.

```md
# 35. Web Catalog Dashboard Handoff
