# 56. Structured Audit Event Pipeline
...
Gateway policy decision과 tool call 결과를 structured audit event로 저장/전송하는 pipeline을 구현한다.

## 주요 소유 경로

- `packages/observability/src/audit/**`
- `apps/gateway/src/audit/**`
...
1. event schema를 구현하거나 보강한다.
2. redaction를 구현하거나 보강한다.
3. batching를 구현하거나 보강한다.
4. retry를 구현하거나 보강한다.
5. sink interface를 구현하거나 보강한다.
...
- audit event가 표준 schema로 생성되고 sink adapter로 전달된다.
...
`docs/handoffs/56_STRUCTURED_AUDIT_PIPELINE.md` 파일을 만들고 다음을 기록한다.

```md
# 56. Structured Audit Event Pipeline Handoff
