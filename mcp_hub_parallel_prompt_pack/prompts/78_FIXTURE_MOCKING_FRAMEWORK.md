# 78. Fixture and Mocking Framework
...
테스트에서 재사용할 fake MCP server, fake IdP, fake upstream framework를 만든다.
...
1. fake MCP server를 구현하거나 보강한다.
2. fake OIDC를 구현하거나 보강한다.
3. fake policy source를 구현하거나 보강한다.
4. scenario builder를 구현하거나 보강한다.
5. docs를 구현하거나 보강한다.
...
- 각 lane의 테스트가 동일한 mock/fixture를 재사용한다.
...
`docs/handoffs/78_FIXTURE_MOCKING_FRAMEWORK.md` 파일을 만들고 다음을 기록한다.

```md
# 78. Fixture and Mocking Framework Handoff
