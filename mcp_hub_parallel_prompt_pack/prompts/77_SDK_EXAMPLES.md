...
MCP server와 Hub API client를 사용하는 예제 코드를 추가한다.
...
1. register server를 구현하거나 보강한다.
2. request grant를 구현하거나 보강한다.
3. call test tool를 구현하거나 보강한다.
4. stdio adapter example를 구현하거나 보강한다.
5. README를 구현하거나 보강한다.
...
- 내부 개발자가 예제를 보고 MCP server를 등록/연동할 수 있다.
...
`docs/handoffs/77_SDK_EXAMPLES.md` 파일을 만들고 다음을 기록한다.

```md
# 77. SDK Examples Handoff
