# 47. First-party Kubernetes Read-only MCP Server
...
read-only Kubernetes 조회용 MCP server를 구현한다.
...
1. namespaces list를 구현하거나 보강한다.
2. pods list/get를 구현하거나 보강한다.
3. events list를 구현하거나 보강한다.
4. no write tools를 구현하거나 보강한다.
5. RBAC manifest를 구현하거나 보강한다.
...
- k8s read-only MCP server가 제한된 service account로 조회 tool만 제공한다.
...
`docs/handoffs/47_FIRST_PARTY_K8S_READONLY_MCP.md` 파일을 만들고 다음을 기록한다.

```md
# 47. First-party Kubernetes Read-only MCP Server Handoff
