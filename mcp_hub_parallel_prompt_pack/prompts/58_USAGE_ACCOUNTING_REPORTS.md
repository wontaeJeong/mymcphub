# 58. Usage and Accounting Reports
...
team/project/server/tool 단위 사용량 집계 job과 API 기초를 구현한다.

## 주요 소유 경로

- `apps/worker/src/accounting/**`
- `packages/observability/src/accounting/**`
...
1. daily aggregate를 구현하거나 보강한다.
2. success/error를 구현하거나 보강한다.
3. risk buckets를 구현하거나 보강한다.
4. CSV export stub를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- 일별 MCP 사용량을 집계하고 추후 비용/감사 보고서로 확장할 수 있다.
...
`docs/handoffs/58_USAGE_ACCOUNTING_REPORTS.md` 파일을 만들고 다음을 기록한다.

```md
# 58. Usage and Accounting Reports Handoff
