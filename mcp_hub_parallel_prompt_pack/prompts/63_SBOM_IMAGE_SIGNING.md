...
SBOM 생성과 image signing/release artifact 검증 절차를 추가한다.
...
1. Syft SBOM를 구현하거나 보강한다.
2. Cosign command stub를 구현하거나 보강한다.
3. digest pinning를 구현하거나 보강한다.
4. verify docs를 구현하거나 보강한다.
...
- 릴리즈 이미지에 SBOM과 signing 절차가 붙는다.
...
`docs/handoffs/63_SBOM_IMAGE_SIGNING.md` 파일을 만들고 다음을 기록한다.

```md
# 63. SBOM and Image Signing Handoff
