# 28. Grant Request and Approval API
...
사용자/팀/프로젝트 단위 MCP 접근 요청과 승인 API를 구현한다.
...
1. request reason를 구현하거나 보강한다.
2. approver를 구현하거나 보강한다.
3. expires_at를 구현하거나 보강한다.
4. allowed_tools를 구현하거나 보강한다.
5. status transition를 구현하거나 보강한다.
6. audit event를 구현하거나 보강한다.
...
- pending grant를 승인/거절할 수 있고 승인된 grant만 Gateway policy source로 노출된다.
...
`docs/handoffs/28_GRANT_APPROVAL_API.md` 파일을 만들고 다음을 기록한다.

```md
# 28. Grant Request and Approval API Handoff
