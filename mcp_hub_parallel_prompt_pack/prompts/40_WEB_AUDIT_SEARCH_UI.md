...
감사 이벤트를 검색하고 상세 trace를 확인하는 UI를 구현한다.
...
1. filters를 구현하거나 보강한다.
2. pagination를 구현하거나 보강한다.
3. event detail drawer를 구현하거나 보강한다.
4. redacted args를 구현하거나 보강한다.
5. trace id link를 구현하거나 보강한다.
...
- 관리자는 누가 어떤 MCP tool을 호출했는지 빠르게 검색할 수 있다.
...
`docs/handoffs/40_WEB_AUDIT_SEARCH_UI.md` 파일을 만들고 다음을 기록한다.

```md
# 40. Audit Search UI Handoff
