...
MCP server catalog 등록/조회/수정/비활성화 API와 DB 모델을 구현한다.
...
1. server slug uniqueness를 구현하거나 보강한다.
2. owner/team를 구현하거나 보강한다.
3. transport type를 구현하거나 보강한다.
4. environment를 구현하거나 보강한다.
5. enabled flag를 구현하거나 보강한다.
6. OpenAPI update를 구현하거나 보강한다.
...
- 관리자는 server catalog를 CRUD할 수 있고 Gateway가 참조 가능한 normalized record가 저장된다.
...
`docs/handoffs/26_CATALOG_CRUD_API.md` 파일을 만들고 다음을 기록한다.

```md
# 26. Server Catalog CRUD API Handoff
