# 55. Metrics and Grafana Dashboards
...
MCP Hub 운영 metrics와 Grafana dashboard JSON을 추가한다.
...
1. calls by server/tool를 구현하거나 보강한다.
2. p95 latency를 구현하거나 보강한다.
3. error rate를 구현하거나 보강한다.
4. denied calls를 구현하거나 보강한다.
5. active sessions를 구현하거나 보강한다.
...
- Prometheus metrics와 기본 dashboard가 추가된다.
...
`docs/handoffs/55_METRICS_DASHBOARDS.md` 파일을 만들고 다음을 기록한다.

```md
# 55. Metrics and Grafana Dashboards Handoff
