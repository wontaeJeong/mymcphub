# 20. Tool Call Policy Interceptor
...
`tools/call` 요청 시점에 policy를 재검증하고 차단/허용/audit decision을 남긴다.
...
1. allow/deny model를 구현하거나 보강한다.
2. deny reason code를 구현하거나 보강한다.
3. argument shape hashing를 구현하거나 보강한다.
4. high-risk tool flag를 구현하거나 보강한다.
5. unit/integration test를 구현하거나 보강한다.
...
- tools/list를 우회한 직접 tool call도 Gateway에서 차단된다.
...
`docs/handoffs/20_TOOL_CALL_POLICY_INTERCEPTOR.md` 파일을 만들고 다음을 기록한다.

```md
# 20. Tool Call Policy Interceptor Handoff
