# 24. MCP Client Onboarding Auth Model
...
opencode/Claude Code/Codex/IDE client를 등록 client와 public client로 구분하는 auth 모델을 구현한다.

## 주요 소유 경로

- `packages/auth/src/client-registry.ts`
- `apps/gateway/src/auth/client.ts`
...
1. client_id registry를 구현하거나 보강한다.
2. redirect URI allowlist를 구현하거나 보강한다.
3. DCR 금지/허용 스위치를 구현하거나 보강한다.
4. client capability metadata를 구현하거나 보강한다.
5. tests를 구현하거나 보강한다.
...
- 허용되지 않은 client_id/redirect_uri 조합은 token 교환 또는 Gateway 접근에서 차단된다.
...
`docs/handoffs/24_MCP_CLIENT_ONBOARDING_AUTH.md` 파일을 만들고 다음을 기록한다.

```md
# 24. MCP Client Onboarding Auth Model Handoff
