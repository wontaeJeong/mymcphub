...
MCP server 상세 페이지를 구현한다.

## 주요 소유 경로

- `apps/web/app/catalog/[serverSlug]/**`
...
1. metadata를 구현하거나 보강한다.
2. tools/resources/prompts를 구현하거나 보강한다.
3. versions를 구현하거나 보강한다.
4. health를 구현하거나 보강한다.
5. grants를 구현하거나 보강한다.
6. audit shortcut를 구현하거나 보강한다.
...
- 서버별 tool 목록, 버전, 권한 상태, health를 상세 확인할 수 있다.
...
`docs/handoffs/36_WEB_SERVER_DETAIL_PAGE.md` 파일을 만들고 다음을 기록한다.

```md
# 36. Server Detail Page Handoff
