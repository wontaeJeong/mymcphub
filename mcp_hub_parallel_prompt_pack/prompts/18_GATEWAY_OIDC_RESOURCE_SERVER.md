# 18. Gateway OIDC Resource Server Middleware
...
Gateway가 remote MCP protected resource처럼 access token을 검증하도록 middleware를 구현한다.
...
1. issuer/audience/expiry 검증를 구현하거나 보강한다.
2. JWKS cache를 구현하거나 보강한다.
3. 401 WWW-Authenticate 응답를 구현하거나 보강한다.
4. scope extraction를 구현하거나 보강한다.
5. 테스트용 fake JWKS를 구현하거나 보강한다.
...
- 유효하지 않은 token은 tools/list 이전에 차단되고, 유효한 token은 user/team/project context로 변환된다.
...
`docs/handoffs/18_GATEWAY_OIDC_RESOURCE_SERVER.md` 파일을 만들고 다음을 기록한다.

```md
# 18. Gateway OIDC Resource Server Middleware Handoff
