...
`tools/list` 응답에서 사용자가 호출할 수 없는 tool을 제거하는 필터를 구현한다.
...
1. server-level grant를 구현하거나 보강한다.
2. tool-level grant를 구현하거나 보강한다.
3. risk level를 구현하거나 보강한다.
4. policy decision trace를 구현하거나 보강한다.
5. snapshot test를 구현하거나 보강한다.
...
- 권한이 없는 tool metadata는 client에 노출되지 않는다.
...
`docs/handoffs/19_TOOL_DISCOVERY_FILTER.md` 파일을 만들고 다음을 기록한다.

```md
# 19. Tool Discovery Filtering Handoff
