...
Hub에 등록되는 MCP server manifest 포맷과 validation을 정의한다.
...
1. server metadata를 구현하거나 보강한다.
2. transport를 구현하거나 보강한다.
3. tools risk hints를 구현하거나 보강한다.
4. secrets를 구현하거나 보강한다.
5. egress를 구현하거나 보강한다.
6. validation CLI를 구현하거나 보강한다.
...
- manifest 파일 하나로 server 등록/검증/배포에 필요한 metadata를 표현할 수 있다.
...
`docs/handoffs/53_MCP_SERVER_MANIFEST_FORMAT.md` 파일을 만들고 다음을 기록한다.

```md
# 53. MCP Server Manifest Format Handoff
