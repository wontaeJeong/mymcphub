# 72. End-to-end MCP Client Tests
...
실제 MCP client 흐름을 모사하는 E2E 테스트를 작성한다.
...
1. fake client를 구현하거나 보강한다.
2. OAuth mock를 구현하거나 보강한다.
3. tools/list를 구현하거나 보강한다.
4. tools/call를 구현하거나 보강한다.
5. deny case를 구현하거나 보강한다.
6. session reconnect를 구현하거나 보강한다.
...
- Gateway를 통해 MCP client가 tools/list/call을 수행하는 전체 흐름이 테스트된다.
...
`docs/handoffs/72_E2E_MCP_CLIENT_TESTS.md` 파일을 만들고 다음을 기록한다.

```md
# 72. End-to-end MCP Client Tests Handoff
