...
MCP Hub 운영 alert rule과 runbook 문서를 작성한다.
...
1. gateway high latency를 구현하거나 보강한다.
2. upstream down를 구현하거나 보강한다.
3. audit sink failure를 구현하거나 보강한다.
4. auth failure spike를 구현하거나 보강한다.
5. deny spike를 구현하거나 보강한다.
...
- PrometheusRule 예시와 장애 대응 runbook이 추가된다.
...
`docs/handoffs/61_ALERT_RULES_RUNBOOKS.md` 파일을 만들고 다음을 기록한다.

```md
# 61. Alert Rules and Runbooks Handoff
