# 23. Step-up Authorization for High-risk Tools
...
high/critical risk tool 호출 전에 추가 승인 또는 재인증 플로우를 위한 서버측 hook을 구현한다.

## 주요 소유 경로

- `apps/gateway/src/auth/**`
- `apps/gateway/src/policy/**`
- `packages/policy/**`
...
1. risk threshold를 구현하거나 보강한다.
2. pending challenge 상태를 구현하거나 보강한다.
3. short-lived approval token를 구현하거나 보강한다.
4. deny fallback를 구현하거나 보강한다.
5. mock UI callback를 구현하거나 보강한다.
...
- critical tool call은 step-up approval 없이는 실행되지 않는다.
...
`docs/handoffs/23_STEP_UP_AUTH_HIGH_RISK_TOOLS.md` 파일을 만들고 다음을 기록한다.

```md
# 23. Step-up Authorization for High-risk Tools Handoff
