...
Gateway와 API의 기본 부하/장시간 테스트 시나리오를 추가한다.
...
1. concurrent sessions를 구현하거나 보강한다.
2. tool call latency를 구현하거나 보강한다.
3. upstream failure를 구현하거나 보강한다.
4. rate limit를 구현하거나 보강한다.
5. report template를 구현하거나 보강한다.
...
- 성능 기준을 수치로 확인할 수 있는 load test 명령이 제공된다.
...
`docs/handoffs/73_LOAD_SOAK_TESTS.md` 파일을 만들고 다음을 기록한다.

```md
# 73. Load and Soak Tests Handoff
