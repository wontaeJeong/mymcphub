...
policy deny 이벤트를 집계해 오탐/권한 요청 후보를 찾는 분석 job을 구현한다.
...
1. deny reason를 구현하거나 보강한다.
2. top denied tools를 구현하거나 보강한다.
3. user/team aggregation를 구현하거나 보강한다.
4. suggested grant candidates를 구현하거나 보강한다.
5. privacy guard를 구현하거나 보강한다.
...
- 운영자는 반복적으로 deny되는 tool과 원인을 볼 수 있다.
...
`docs/handoffs/59_DENIED_CALL_ANALYTICS.md` 파일을 만들고 다음을 기록한다.
