...
서버/tool/version/grant 단위 emergency disable API를 구현한다.
...
1. disable server를 구현하거나 보강한다.
2. disable tool를 구현하거나 보강한다.
3. disable version를 구현하거나 보강한다.
4. revoke grants를 구현하거나 보강한다.
5. reason required를 구현하거나 보강한다.
6. audit를 구현하거나 보강한다.
...
- 관리자가 특정 MCP server 또는 tool을 즉시 차단할 수 있고 Gateway가 이를 반영한다.
...
`docs/handoffs/32_ADMIN_KILL_SWITCH_API.md` 파일을 만들고 다음을 기록한다.

```md
# 32. Admin Kill Switch API Handoff
