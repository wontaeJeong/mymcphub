...
const serverInfo = { name: "k8s-readonly", version: "0.1.0" } as const;

const namespaces = ["default", "platform", "observability"] as const;

export const mockPods: MockPod[] = [
  { namespace: "default", name: "web-7d9c5f", phase: "Running", nodeName: "kind-worker", labels: { app: "web" } },
  { namespace: "platform", name: "mcp-gateway-6f8b9", phase: "Running", nodeName: "kind-control-plane", labels: { app: "mcp-gateway" } },
  { namespace: "observability", name: "otel-collector-0", phase: "Pending", nodeName: "kind-worker", labels: { app: "otel" } }
];

export const k8sReadonlyTools: ToolDefinition[] = [
  {
    name: "list_namespaces",
    description: "List namespace names from the local read-only mock Kubernetes dataset.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    call() {
      return textJsonResult({ namespaces: [...namespaces] });
    }
  },
  {
    name: "list_pods",
    description: "List pods in one namespace from the local read-only mock Kubernetes dataset.",
    inputSchema: {
      type: "object",
    ...
    call(argumentsJson) {
      const namespace = readRequiredString(argumentsJson.namespace, "namespace");
      assertNamespace(namespace);
      return textJsonResult({ pods: mockPods.filter((pod) => pod.namespace === namespace) });
    }
  },
  {
    name: "get_pod",
    description: "Read one pod by namespace and name from the local read-only mock Kubernetes dataset.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    call(argumentsJson) {
      const namespace = readRequiredString(argumentsJson.namespace, "namespace");
      const podName = readRequiredString(argumentsJson.podName, "podName");
      assertNamespace(namespace);
      const pod = mockPods.find((candidate) => candidate.namespace === namespace && candidate.name === podName);
      if (!pod) {
        throw new ToolInputError(`Unknown pod: ${namespace}/${podName}`);
      }

      return textJsonResult({ pod });
    }
  }
];
...
function handleParsedMcpRequest(request: JsonRpcRequest): JsonRpcResponse {
  ...
  if (request.method === "tools/list") {
    return result(request.id, {
      tools: k8sReadonlyTools.map(({ name, description, inputSchema }) => ({ name, description, inputSchema }))
    });
  }
  ...
  if (request.method === "resources/list") {
    return result(request.id, {
      resources: namespaces.map((namespace) => ({ uri: `k8s-readonly://namespaces/${namespace}`, name: namespace }))
    });
  }
...
export function startK8sReadonlyServer(port = Number(process.env.PORT ?? 5102)) {
...
async function handleHttpRequest(request: IncomingMessage, response: ServerResponse) {
  ...
  if (request.method === "GET" && url.pathname === "/health") {
    sendJson(response, 200, { status: "ok", server: serverInfo.name, mode: "mock" });
    return;
  }
...
function callTool(request: JsonRpcRequest): JsonRpcResponse {
  ...
  const tool = k8sReadonlyTools.find((candidate) => candidate.name === toolName);
  if (!tool) {
...
function assertNamespace(namespace: string) {
  if (!namespaces.includes(namespace as (typeof namespaces)[number])) {
    throw new ToolInputError(`Unknown namespace: ${namespace}`);
  }
}
